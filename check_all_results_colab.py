...
# -*- coding: utf-8 -*-
"""
批量检查所有Image Matching结果文件（Colab版本）
在Colab中直接运行此脚本
"""
...
def check_single_file(result_file):
    ...
    try:
        ...
        if 'results' not in data:
            return {'status': 'ERROR', 'error': '缺少results字段'}
        ...
        if missing_fields:
            return {'status': 'ERROR', 'error': f'缺少字段: {missing_fields}'}
        ...
        return {'status': 'ERROR', 'error': f'读取错误: {e}'}
    ...
    if not json_files:
        ...
        for json_file in json_files:
            ...
            if status == 'OK':
                ...
                print(f"   大小: {size_mb:.1f}MB, 查询: {result['num_queries']}, 匹配: {result['num_matches']}")
                ...
                if 'error' in result:
                    ...
                    print(f"   查询: {result['num_queries']}, 匹配: {result['num_matches']}")
                    print(f"   正确预测平均inliers: {result['correct_mean_inliers']:.1f}")
                    print(f"   错误预测平均inliers: {result['incorrect_mean_inliers']:.1f}")
